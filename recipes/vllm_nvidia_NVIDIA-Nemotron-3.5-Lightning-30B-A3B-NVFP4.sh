...
EXTRA_ARGS="--max-num-seqs 256 --max-num-batched-tokens 16384 --enable-prefix-caching --async-scheduling --mamba-backend flashinfer --moe-backend humming --linear-backend humming --mamba-ssu-algorithm horizontal --mamba-cache-mode align --mamba-ssm-cache-dtype float16 --enable-mamba-cache-stochastic-rounding --mamba-cache-philox-rounds 5"
