...
echo ""
echo "🚀 GLM 4.7 Flash Inference Server Launcher"
...
main() {
    ...
    echo "Port: $VLLM_PORT"
    echo ""
    local base_command="vllm serve $MODEL_REPO --tensor-parallel-size $TENSOR_PARALLEL_SIZE --speculative-config.method mtp --speculative-config.num_speculative_tokens 1 --tool-call-parser glm47 --reasoning-parser glm45 --enable-auto-tool-choice --max-model-len auto --served-model-name $MODEL_NAME --port $VLLM_PORT --api-key YOUR_API_KEY"
    if [ "$GPU_SELECTION_MODE" = "custom" ]; then
