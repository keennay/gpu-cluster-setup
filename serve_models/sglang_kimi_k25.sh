...
main() {
    ...
    echo ""
    local base_command="sglang serve --model-path $MODEL_REPO --tp $TENSOR_PARALLEL_SIZE --trust-remote-code --tool-call-parser $MODEL_NAME --reasoning-parser $MODEL_NAME --context-length 49152 --host 0.0.0.0 --port $INFERENCE_PORT --api-key YOUR_API_KEY"
    if [ "$GPU_SELECTION_MODE" = "custom" ]; then
