...
main() {
    ...
    echo ""
    local base_command="python -m sglang.launch_server --model-path $MODEL_REPO --tp-size $TENSOR_PARALLEL_SIZE --tool-call-parser $MODEL_NAME --reasoning-parser minimax-append-think --host 0.0.0.0 --trust-remote-code --port $INFERENCE_PORT --mem-fraction-static 0.85 --api-key YOUR_API_KEY"
    if [ "$GPU_SELECTION_MODE" = "custom" ]; then
